...
def generate_visualizations(model, test_dataset, output_dir, n_way=5, k_shot=5, n_query=15, device=torch.device("cpu")):
    ...
    plot_embeddings_tsne(all_embeddings, all_labels, results['prototypes'],
                      ...
                      save_path=os.path.join(output_dir, f"embedding_space_{n_way}way_{k_shot}shot.png"))
    
    # 2. Confusion matrix
    plot_confusion_matrix(results['query_labels'], results['query_predictions'],
                      title=f"{n_way}-way {k_shot}-shot Confusion Matrix",
                      save_path=os.path.join(output_dir, f"confusion_matrix_{n_way}way_{k_shot}shot.png"))
    
    # 3. Distance heatmap
    visualize_distance_heatmap(results['query_embeddings'], results['prototypes'],
                            results['query_labels'],
                            save_path=os.path.join(output_dir, f"distance_heatmap_{n_way}way_{k_shot}shot.png"))
...
def main():
    ...
    parser.add_argument('--backbone', type=str, default='efficientnet_v2_s', choices=['resnet50', 'efficientnet_v2_s'], 
    ...
    plot_shot_comparison(shot_results,
                        save_path=os.path.join(args.output_dir, f"shot_comparison_{args.n_way}way.png"))
